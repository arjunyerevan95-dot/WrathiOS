...
DERIVED_DATA="$ROOT_DIR/DerivedData/Gate5C"
ARTIFACT_DIR="$ROOT_DIR/Artifacts/gate5c-semantic-menu"
LOG="$ARTIFACT_DIR/xcodebuild.log"
PROJECT="$ROOT_DIR/WrathiOSGate5C.xcodeproj"
APP_BUNDLE="$DERIVED_DATA/Build/Products/Debug-iphoneos/WrathiOSGate5C.app"
BINARY="$APP_BUNDLE/WrathiOSGate5C"
PACKAGE_ROOT="$ROOT_DIR/Derived/gate5c-package"
IPA="$ARTIFACT_DIR/WrathiOSGate5C-v11-unsigned.ipa"
...
cd "$ROOT_DIR"
python3 scripts/materialize_gate5c_menu_qc.py
WRATH_ENGINE_BUILD_FLAVOR=gate5c python3 scripts/build_gate2_engine_archive.py
xcodegen generate --spec project-gate5c.yml

set -o pipefail
xcodebuild \
    -project WrathiOSGate5C.xcodeproj \
    -scheme WrathiOSGate5C \
    -configuration Debug \
    -sdk iphoneos \
...
required_symbols=(
    ...
    '_WrathIOSGate5CSnapshotBegin$'
    '_WrathIOSGate5CSnapshotEntry$'
    '_WrathIOSGate5CSnapshotEnd$'
    '_WrathIOSGate5CNextButtonEvent$'
)
...
for marker in \
    ...
    grep -Fq "$marker" "$ARTIFACT_DIR/strings.txt" || {
        echo "error: Gate 5C binary is missing semantic adapter marker: $marker" >&2
        exit 1
    }
done
grep -Fq 'wrathios-menu.dat' "$ARTIFACT_DIR/bundle-inventory.txt" || {
...
[[ "$short_version" == '0.0.11' && "$build_version" == '11' ]] || {
...
ditto "$APP_BUNDLE" "$PACKAGE_ROOT/Payload/WrathiOSGate5C.app"
rm -rf "$PACKAGE_ROOT/Payload/WrathiOSGate5C.app/_CodeSignature"
rm -f "$PACKAGE_ROOT/Payload/WrathiOSGate5C.app/embedded.mobileprovision"
(
    cd "$PACKAGE_ROOT"
...
cp "$BINARY" "$ARTIFACT_DIR/WrathiOSGate5C-arm64"
cp docs/GATE5C_DEVICE_CHECKLIST.md "$ARTIFACT_DIR/device-test-checklist.md"
cp docs/GATE5C_SEMANTIC_MENU_TOUCH.md "$ARTIFACT_DIR/semantic-menu-call-graph.md"
cp Artifacts/gate5c-qc-patches/report.json "$ARTIFACT_DIR/qc-patch-report.json"
cp Artifacts/engine-patches/report.json "$ARTIFACT_DIR/engine-patch-report.json"
cp Artifacts/gate5c-engine-archive/report.json "$ARTIFACT_DIR/engine-archive-report.json"

ipa_size="$(stat -f '%z' "$IPA")"
ipa_sha256="$(shasum -a 256 "$IPA" | awk '{print $1}')"
cat > "$ARTIFACT_DIR/summary.md" <<EOF
# Gate 5C semantic menu-touch device build
...
- Semantic UIKit adapter and authentic pointer-hover-click symbols: present
- Derived menu QC source: pinned and provenance-checked
- Semantic bounds: emitted by authentic QC hover geometry
...
- Profile keyboard, gyro, gameplay swipe, movement, and firing: absent by scope
- Physical direct-touch result: device-unverified
EOF

cat "$ARTIFACT_DIR/summary.md"
cat "$ARTIFACT_DIR/SHA256SUMS.txt"
echo "Gate 5C semantic menu device build and packaging validation passed"
