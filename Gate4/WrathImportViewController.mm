...
- (void)viewDidLoad {
    ...
    self.chooseButton.configuration = [UIButtonConfiguration filledButtonConfiguration];
    self.chooseButton.configuration.title = @"Choose WRATH Folder";
    self.chooseButton.configuration.cornerStyle = UIButtonConfigurationCornerStyleLarge;
    self.chooseButton.configuration.baseBackgroundColor = [UIColor colorWithRed:0.15 green:0.48 blue:0.96 alpha:1.0];
    ...
    self.removeButton.configuration = [UIButtonConfiguration borderedButtonConfiguration];
    self.removeButton.configuration.title = @"Remove Imported Data";
    self.removeButton.configuration.cornerStyle = UIButtonConfigurationCornerStyleLarge;
    self.removeButton.configuration.baseForegroundColor = [UIColor colorWithRed:1.0 green:0.42 blue:0.42 alpha:1.0];
...
- (void)refreshInstalledState {
    ...
    [[WrathDataImporter sharedImporter] inspectInstalledDataWithCompletion:^(WrathImportReport *report, NSError *error) {
        ...
        if (report.compatible) {
            self.stateLabel.text = @"Licensed WRATH data is ready";
            ...
            self.stateLabel.text = @"No compatible data imported";
            self.stateLabel.textColor = UIColor.whiteColor;
            self.detailLabel.text = error.localizedFailureReason.length > 0
                ? error.localizedFailureReason
                : @"Select a licensed WRATH installation. Required sentinels: progs.dat, csprogs.dat, and menu.dat.";
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls {
    ...
    [[WrathDataImporter sharedImporter] importFromSelectedDirectory:selected
        ...
        [self setBusy:NO message:@""];
        if (error != nil) {
            self.stateLabel.text = @"Import failed";
        ...
        self.stateLabel.text = @"Import validated and installed";
        self.stateLabel.textColor = [UIColor colorWithRed:0.3 green:0.95 blue:0.56 alpha:1.0];
        self.detailLabel.text = [NSString stringWithFormat:@"%@\n%@", report.summary, report.details];
        self.removeButton.hidden = NO;
    }];
